...
const SimilarProductsSection = ({ categoryId, activeProductId }) => {
    ...
    const similarTotalPages = useSelector(
        (state) => state.product.similarTotalPages,
    );
    const similarProductsLoading = useSelector(
        (state) => state.product.similarProductsLoading,
    );
    const similarProductsError = useSelector(
        (state) => state.product.similarProductsError,
    );
    ...
    return (
        <section className="similar-products">
                ...
                {similarTotalPages > 1 && (
                    <div className="similar-products__nav">
                        <button
                            type="button"
                            onClick={() =>
                                setSimilarPage((prev) => Math.max(prev - 1, 1))
                            }
                            disabled={
                                similarPage === 1 || similarProductsLoading
                            }
                        ...
                        <button
                            ...
                            onClick={() =>
                                setSimilarPage((prev) =>
                                    Math.min(prev + 1, similarTotalPages),
                                )
                            }
                            disabled={
            ...
            <div
                className={`similar-products__grid ${similarProductsLoading ? 'similar-products__grid--loading' : ''}`}
            >
